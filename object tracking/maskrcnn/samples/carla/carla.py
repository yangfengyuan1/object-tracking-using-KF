...
train_csv_path = '/mnt/datasets/CARLA_camera/train.csv'
val_csv_path = '/mnt/datasets/CARLA_camera/val.csv'
test_csv_path = '/mnt/datasets/CARLA_camera/test.csv'
...
class carlaDataset(utils.Dataset):
    ...
    def load_images(self, csv_path):
        with open(csv_path,'r') as f:
            images_path = [os.path.join(os.path.dirname(csv_path),\
                row['prefix'],'rgb','{}.png'.format(row['id'])) \
        ...
        for image_path in images_path:
            print("[image]",image)
            self.add_image('carla', image_id=image_path, path=image_path)
    ...
    def load_mask(self, image_id):
        ...
        img_name = os.path.basename(info['id'])
        mask_path = os.path.join(info['id'],'../../sem_img',img_name)
        mask_label = skimage.io.imread(mask_path)
        # # If grayscale. Convert to RGB for consistency.
    ...
    def get_labels(self, labels):
        ...
        unlabeled_index = np.all(labels == 0, axis=-1)
        building_index = np.all(labels == 1, axis=-1)
        fence_index = np.all(labels == 2, axis=-1)
        other_index = np.all(labels == 3, axis=-1)
        pedestrian_index = np.all(labels == 4, axis=-1)
        pole_index = np.all(labels == 5, axis=-1)
        road_line_index = np.all(labels == 6, axis=-1)
        road_index = np.all(labels == 7, axis=-1)
        sidewalk_index = np.all(labels == 8, axis=-1)
        vegetation_index = np.all(labels == 9, axis=-1)
        car_index = np.all(labels == 10, axis=-1)
        wall_index = np.all(labels == 11, axis=-1)
        traffic_sign_index = np.all(labels == 12, axis=-1)
...
train = carlaDataset()
train.load_images(train_csv_path)
train.prepare()
val = carlaDataset()
val.load_images(val_csv_path)
val.prepare()
...
config = CarlaConfig()
config.STEPS_PER_EPOCH = len(train.image_info)//config.BATCH_SIZE
config.VALIDATION_STEPS = len(val.image_info)//config.BATCH_SIZE
config.display()
